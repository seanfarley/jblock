...
class JBlockBucket:
    ...
    HITLIST_DICT_TYPE = typing.Dict[
        typing.Tuple["JBlockBucket", str], typing.List[parser.JBlockRule]
    ]
    ...
    __slots__ = [
        ...
        "length",
        "domain_hitlist",
        "domain_exceptionlist",
    ]  # type: typing.List[str]

    def __init__(
        self,
        rules: typing.MutableSequence[parser.JBlockRule],
        hitlist_dict: HITLIST_DICT_TYPE,
        exceptionlist_dict: HITLIST_DICT_TYPE,
        ...
        for r in rules:
            if (
                r.matcher is None
                or (r.matcher.dummy_matcher() and not r.options)
                or not r.matching_supported(self.supported_options)
            ):
                continue
        ...
        for variant in domain_variants:
            ...
            exceptions_in_flight.update(
                itertools.filterfalse(
                    functools.partial(operator.contains, rules_in_flight),
            ...
            rules_in_flight.update(
                itertools.filterfalse(
                    functools.partial(operator.contains, exceptions_in_flight),
...
ie: an accept and a fail bucket, all with one tag.
    ...
    __slots__ = ["bucket_token", "blacklist", "whitelist"]  # type: typing.List[str]

    def __init__(
        self,
        bucket_token: jtoken.Token,
...
class JBlockBuckets:
    ...
    def __init__(
        ...
        token_frequency: "typing.Counter[jtoken.Token]" = None,
    ) -> None:
        self.supported_options = supported_options
        if token_frequency:
    ...
    def _rule_list_to_bucket_group(
        self, bucket_token: jtoken.Token, rule_list: typing.List[parser.JBlockRule]
    ) -> JBlockBucketGroup:
        ...
        return JBlockBucketGroup(
            bucket_token,
            JBlockBucket(blacklist, *self.buckets_vars),
            JBlockBucket(whitelist, *self.buckets_vars),
        )
    ...
    def _pick_token(self, rule):
        ...
        if self.token_frequency:
            ...
            # rule!
            return min(
                tokens, default=None, key=lambda k: self.token_frequency.get(k, 0)
            )
        else:
            if tokens:
                return tokens[0]
    ...
    def reset_token_frequency(self):
        ...
        self.token_frequency = (
            collections.Counter()
        )  # type: typing.Counter[jtoken.Token]
    ...
    def should_block(self, url: str, options=frozenset()) -> bool:
        ...
        if not block:
            block = self.fallback_bucket_group.blacklist.hit(
                url, domain_variants, options
            )
        ...
        for t in tokens:
            ...
            if group is not None and group.whitelist.hit(url, domain_variants, options):
                ...
                break
        if block:
            block = not self.fallback_bucket_group.whitelist.hit(
                url, domain_variants, options
            )
    ...
    def __len__(self):
        return sum(map(len, self.bucket_groups.values())) + len(
            self.fallback_bucket_group
        )
